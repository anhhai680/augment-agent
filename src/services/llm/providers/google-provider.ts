...
export class GoogleProvider implements LLMProvider {
  ...
  private async makeRequest(endpoint: string, body?: any, method: 'GET' | 'POST' = 'POST'): Promise<any> {
    ...
    if (!response.ok) {
      ...
      throw new Error(`Google API request failed: ${response.status} ${response.statusText} - ${errorData.error?.message || 'Unknown error'}`);
    }
